...
;| Variables 15/20 |;
$INPUT      ROW ID NOMTIME TIME EVID CMT AMT DV FLAG STUDY BLQ
            CYCLE=DROP DOSE PART PROFDAY PROFTIME BBW eff0
$DATA      xgxr1.csv IGNORE=@ IGNORE=(FLAG.NE.0)
...
$ESTIMATION METHOD=1 POSTHOC INTER MAXEVAL=9999 NSIG=2 SIGL=9 PRINT=10
            NOABORT
$TABLE      NOPRINT FILE=xgxr028_res.txt ONEHEADER ROW ; ROW ; row identifier
