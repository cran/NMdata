$PROBLEM    032 with between occasion var
...
;;; BOV/IOV/between occasion variability
IF(TIME.LE.2) KA=KA*EXP(ETA(6))
IF(TIME.GT.2) KA=KA*EXP(ETA(7))
IF(TIME.GT.5) KA=KA*EXP(ETA(8))
...
;;            PRINT=10 NOABORT MSFO=xgxr044.msf
...
$TABLE ROW KA V2 V3 CL Q PRED IPRED Y NOPRINT FILE=xgxr044_res.txt
